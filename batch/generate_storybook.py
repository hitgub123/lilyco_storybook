...
def get_browser(playwright: Playwright):
    ...
    if not target_page:
        logging.error(f"错误：在所有打开的标签页中，没有找到包含 '{TARGET_URL_PART}' 的页面。")
        logging.error("请确认你已经在指定的浏览器窗口中手动打开并登录了目标网站。")
        browser.close()
        return
    logging.info(f"连接成功！当前页面是: {page.title()}")
    return browser,context,target_page


def run(browser, context, target_page, prompt):
    # ---. 连接到在9222端口上运行的现有浏览器 ---
    # 注意：这里不再是 launch()，而是 connect_over_cdp()
    logging.info("正在连接到已打开的浏览器...")
    browser = playwright.chromium.connect_over_cdp("http://localhost:9222")
    ...
    logging.info(f"连接成功！当前页面是: {target_page.title()}")
    ...
    try:
        ...
        logging.info("正在等待share_button元素加载...")
        ...
        while 1:
            ...
            logging.info(f"操作成功！截图{screenshot_path}已保存。")
            current_page += 1
...
if __name__ == "__main__":
    ...
    with sync_playwright() as playwright:
        ...
        run(browser, context, target_page, prompt)