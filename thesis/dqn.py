...
import numpy as np,time,cv2,tensorflow as tf,math
import os,sys
...
class DeepQNetwork:
    ...
    def train(self):
        ...
        while 1:
            ...
            # total_memory = self.memory_count + self.memory_plus_count + self.memory_minus_count
            # if (total_memory >= max(self.batch_size, self.OBSERVE)) and (self.step % self.learn_step == 0):
            #     # 开始学习，调整 FRAME_PER_ACTION / epsilon 初始值等
            # self.learn()
            ...
            if self.step%500==0:
                print("TIMESTEP", self.step, "/ STATE", state,"/ EPSILON", self.epsilon, "/ REWARD", reward,"/ memory_plus_count",self.memory_plus_count)
            ...
            if (
                state == "train"
                and self.step % self.save_model_step == self.save_model_step - 1
            ):
                self.save_model()
